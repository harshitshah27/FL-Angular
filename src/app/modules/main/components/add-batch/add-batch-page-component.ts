...
@Component({
  selector: 'add-batch-new',
  templateUrl: './add-batch-page.component.html',
  styleUrls: ['./add-batch-page.component.scss']
})
export class AddBatchPageComponent implements OnInit {
  ...
  financeForm = this.fb.group({
    stepperOne: this.fb.group({
      sold_to_name: ['', [Validators.required]],
      sales_unit: ['', [Validators.required]],
      region: ['', [Validators.required]]
    }),
    finalise_entire_transaction: [true],
    dispatch_details: this.fb.array([]),
  })
  stepperOne = (this.financeForm.get('stepperOne') as FormGroup);
  dispatchDetails = this.financeForm.get('dispatch_details') as FormArray
